...
export default function MeUpsertModal({
  isOpen,
  setIsOpen,
}: MeUpsertModalParams) {
...
function MeUpsertForm({
  ...
  useEffect(() => {
    ...
    if (me) {
      setLogoBase64Encoded(me.avatar)
    }
  }, [me])

  async function submitForm() {
    const formValues = await meForm.validateFields()
    formValues.avatar = logoBase64Encoded
    const { userId } = me
    ...
    >
      ...
      >
        <Upload
          ...
          defaultFileList={initialValues?.avatar ? [initialValues.avatar] : undefined}
        >
